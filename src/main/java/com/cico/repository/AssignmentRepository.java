...
//			PageRequest pageRequest);
	...
	@Query("SELECT NEW com.cico.payload.AssignmentSubmissionResponse(ts.student.applyForCourse, ts.student.fullName, ts.submissionDate, ts.status, ts.student.profilePic, a.title, ts.submitFile, ts.description, ts.submissionId, ts.review, a.id, t.taskNumber) " +
		...
		Page<AssignmentSubmissionResponse> findAllAssignmentSubmissionWithCourseIdAndSubjectId(
		        @Param("courseId") Integer courseId, @Param("subjectId") Integer subjectId, @Param("status") SubmissionStatus status,
		        PageRequest pageRequest);
	...
	@Query("SELECT  "
			...
			+ "FROM Assignment a " + "JOIN a.AssignmentQuestion t " + "  JOIN t.assignmentSubmissions ts "
			+ " WHERE ts.status IN ('Unreviewed' ,'Reviewing' ,'Rejected', 'Accepted' )" + "AND t.isDeleted = 0  AND a.isDeleted =0 AND a.id=:assignmentId "
			+ " GROUP BY  ts.id ,t.id ")
	...
	Page<Assignment>findAllByCourseIdAndSubjectIdAndIsDeletedFalse(Integer courseId, Integer subjectId, PageRequest of);
	...
	@Query("""
		...
		""")
		Page<AssignmentAndTaskSubmission> findAllAssignmentStatusWithAssignmentId(
