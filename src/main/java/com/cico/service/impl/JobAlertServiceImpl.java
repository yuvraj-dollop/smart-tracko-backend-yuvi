...
//	
	...
	public JobAlertResponse update(UpdateJobRequest request) {
	    ...
	    alert.setJobDescription(request.getJobDescription() != null ? request.getJobDescription() : alert.getJobDescription());
	    
	    alert.setCompanyName(request.getCompanyName() != null ? request.getCompanyName() : alert.getCompanyName());
	    
	    alert.setExperienceRequired(request.getExperienceRequired() != null ? request.getExperienceRequired() : alert.getExperienceRequired());
	    
	    alert.setTechnicalSkills(request.getTechnicalSkills() != null ? request.getTechnicalSkills() : alert.getTechnicalSkills());
	    ...
	    alert.setTechnologyStack(
	        request.getTechnologyStackId() != null
	...
	public PageResponse<JobAlertResponse> getAllJobAlert(int page, int size, JobType type) {
	    ...
	    List<JobAlertResponse> jobAlertResponses = jobAlertPage.getContent()
	        .stream()
	        .map(this::mapToJobAlertResponse)
	        .toList();

	    return new PageResponse<>(
	        jobAlertResponses,
	        jobAlertPage.getNumber(),
	        jobAlertPage.getSize(),
	        jobAlertPage.getTotalElements(),
	        jobAlertPage.getTotalPages(),
	        jobAlertPage.isLast()
	    );
	}
	...
	public PageResponse<JobAlertResponse> getAllJobsAndIntership(Integer page, Integer size) {
	    ...
	    List<JobAlertResponse> responseList = jobAlertPage.getContent()
	        .stream()
	        .map(this::mapToJobAlertResponse)
	        .toList();
	...
	public List<JobAlertResponse> searchJob(String field, String role) {
		List<JobAlert> jobs = role.equalsIgnoreCase("student")
			? repository.jobSearchStudent(field)
			: repository.jobSearchAdmin(field);
	...
	private JobAlertResponse mapToJobAlertResponse(JobAlert jobAlert) {
	    return JobAlertResponse.builder()
	            .jobId(jobAlert.getJobId())
	            .jobTitle(jobAlert.getJobTitle())
	            .jobDescription(jobAlert.getJobDescription())
	            .companyName(jobAlert.getCompanyName())
	            .experienceRequired(jobAlert.getExperienceRequired())
	            .technicalSkills(jobAlert.getTechnicalSkills())
	            .jobPackage(jobAlert.getJobPackage())
	            .isActive(jobAlert.getIsActive())
	            .technologyStackName(
	                jobAlert.getTechnologyStack() != null ? jobAlert.getTechnologyStack().getTechnologyName() : null
	            )
	            .type(jobAlert.getType())
	            .createdDate(jobAlert.getCreatedDate())
	            .updatedDate(jobAlert.getUpdatedDate())
	            .build();
	}
}